...
from dopamine.agents.dqn.models.CNNV2 import CNNV2DQN
...
class DQNAgent(object):
    ...
    """
        ...
        self.model_creator = CNNV2DQN(num_actions=self.num_actions)
        ...
        with tf.device(tf_device):
            ...
            # self._dqn_model = CNNOriginal(num_actions=self.num_actions).build_networks(
            #     state_ph=self.state_ph,
            #     _replay=self._replay
            # )

            self._dqn_model = self.model_creator.build_networks(
                state_ph=self.state_ph,
                _replay=self._replay
            )
    ...
    """
        ...
        replay_next_qt_max = tf.reduce_max(
            self._dqn_model.replay_next_target_net_outputs.q_values, 1)
    ...
    """
        ...
        replay_chosen_q = tf.reduce_sum(
            self._dqn_model.replay_net_outputs.q_values * replay_action_one_hot,
            reduction_indices=1,
    ...
    """
        ...
        if random.random() <= epsilon:
            ...
            return self._sess.run(self._dqn_model.q_argmax, {self.state_ph: self.state})
    ...
    """
        ...
        if self.training_steps % 1000 == 0:
            logger.debug('_train_step = {}'.format(self.training_steps))

        if self._replay.memory.add_count > self.min_replay_history:
            if self.training_steps % self.update_period == 0:
                self._sess.run(self._train_op)
                logger.debug('Running _train_op')
                logger.debug('summary_writter {} {} {}'.format(self.summary_writing_frequency,
                                                               self.summary_writer,
                                                               self.training_steps))
                if (self.summary_writer is not None and
                    ...
                    logger.debug('add_summary! global_step = {}'.format(self.training_steps))
