...
class CNNV2DQN(ModelCreator):
    ...
    """
        ...
        logger.info('net_before_reshape {}'.format(net.get_shape()))

        if len(state.get_shape()) == 5:
            # We then have to merge the history dimension with channels in the image
            print('Need reshaping!')
            net = tf.reshape(net, [net.shape[0], net.shape[1], net.shape[2], -1])
