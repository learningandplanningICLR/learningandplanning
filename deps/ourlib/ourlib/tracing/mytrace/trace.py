    ...
    def write_results(self, show_missing=True, summary=False, coverdir=None):
        ...
        if self.calledfuncs:
            ...
            for filename, modulename, funcname in sorted(calls):
                print(("filename: %s, modulename: %s, funcname: %s"
                       % (filename, modulename, funcname)), file=self.output_file)
                self.maybe_flush()
        ...
        if self.callers:
            ...
            for ((pfile, pmod, pfunc), (cfile, cmod, cfunc)) \
                ...
                if cfile != pfile and lastcfile != cfile:
                    ...
                    self.maybe_flush()
                    lastcfile = cfile
                print("    %s.%s -> %s.%s" % (pmod, pfunc, cmod, cfunc), file=self.output_file)
                self.maybe_flush()
        ...
        if summary and sums:
            ...
            for m in sorted(sums):
                ...
                print("%5d   %3d%%   %s   (%s)" % sums[m], file=self.output_file)
                self.maybe_flush()
...
class AwarelibTrace:
                    ...
                    if not ignore_it:
                        if self.trace:
                            print((" --- modulename: %s, funcname: %s"
                                   % (modulename, code.co_name)), file=self.output_file)
                            self.maybe_flush()
    ...
    def localtrace_trace_and_count(self, frame, why, arg):
        if why == "line":
            ...
            if self.start_time:
                print('%.2f' % (_time() - self.start_time), end=' ', file=self.output_file)
                self.maybe_flush()

            bname = os.path.basename(filename)
            print("%s(%d): %s" % (bname, lineno,
                                  linecache.getline(filename, lineno)), end='', file=self.output_file)
            self.maybe_flush()
        return self.localtrace
